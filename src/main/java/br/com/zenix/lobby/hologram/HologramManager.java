...
import br.com.zenix.lobby.hologram.PlayerTop.PlayerTopClans;
...
public class HologramManager extends Management {
	...
	private static final List<PlayerTopClans> topScoreClans = new ArrayList<>();
	...
	public boolean updateHolograms() {
		...
		loadListClans(topScoreClans);
		return setHolograms();
	}

	public boolean loadList(List<PlayerTop> list) {
		ResultSet set = getManager().getCoreManager().getDataManager().getMySQL()
				.executeQuery("SELECT * FROM `global_data` WHERE `type`=12 ORDER BY `value` DESC LIMIT 15");
		try {
			int id = 0;
			while (set.next()) {
				id++;
	...
	public boolean loadListHG(List<PlayerTopHGWins> list) {
		ResultSet set = getManager().getCoreManager().getDataManager().getMySQL()
				.executeQuery("SELECT * FROM `global_data` WHERE `type`=5 ORDER BY `value` DESC LIMIT 15");
		try {
			int id = 0;
			while (set.next()) {
				id++;
				list.add(new PlayerTopHGWins(id,
	...
	public boolean loadListClans(List<PlayerTopClans> list) {
		ResultSet set = getManager().getCoreManager().getDataManager().getMySQL()
				.executeQuery("SELECT * FROM `znx_clan_data` ORDER BY `xp` DESC LIMIT 15");
		try {
			int id = 0;
			while (set.next()) {
				id++;
				list.add(new PlayerTopClans(id, set.getString("name"), set.getInt("xp"),
						getManager().getCoreManager().getClanManager().getClan(set.getString("name"), "elo")));
			}
			set.close();
			return true;
		} catch (Exception exeption) {
	...
	public boolean loadListPvP(List<PlayerTopPvP> list) {
		ResultSet set = getManager().getCoreManager().getDataManager().getMySQL()
				.executeQuery("SELECT * FROM `global_data` WHERE `type`=8 ORDER BY `value` DESC LIMIT 15");
		try {
			int id = 0;
			while (set.next()) {
				id++;
				list.add(new PlayerTopPvP(id,
						getManager().getCoreManager().getNameFetcher().getName(set.getInt("player")),
						set.getInt("value")));
			}
			set.close();
	...
	public boolean loadListSWELO(List<PlayerTopSWElo> list) {
		ResultSet set = getManager().getCoreManager().getDataManager().getMySQL()
				.executeQuery("SELECT * FROM `global_data` WHERE `type`=40 ORDER BY `value` DESC LIMIT 15");
		try {
			int id = 0;
			while (set.next()) {
				id++;
				list.add(new PlayerTopSWElo(id,
						getManager().getCoreManager().getNameFetcher().getName(set.getInt("player")),
						set.getInt("value")));
			}
			set.close();
	...
	public boolean loadListSWWins(List<PlayerTopSWWins> list) {
		ResultSet set = getManager().getCoreManager().getDataManager().getMySQL()
				.executeQuery("SELECT * FROM `global_data` WHERE `type`=39 ORDER BY `value` DESC LIMIT 15");
		try {
			int id = 0;
			while (set.next()) {
				id++;
				list.add(new PlayerTopSWWins(id,
						getManager().getCoreManager().getNameFetcher().getName(set.getInt("player")),
						set.getInt("value")));
			}
			set.close();
	...
	public boolean loadListSWDeaths(List<PlayerTopSWDeaths> list) {
		ResultSet set = getManager().getCoreManager().getDataManager().getMySQL()
				.executeQuery("SELECT * FROM `global_data` WHERE `type`=38 ORDER BY `value` DESC LIMIT 15");
		try {
			int id = 0;
			while (set.next()) {
				id++;
				list.add(new PlayerTopSWDeaths(id,
						getManager().getCoreManager().getNameFetcher().getName(set.getInt("player")),
						set.getInt("value")));
			}
			set.close();
	...
	public boolean setHolograms() {
		...
		Location scoreLocation = new Location(Bukkit.getServer().getWorld("world"), -1, 66, -40);
		scoreHologram = new Hologram("§e§lTOP 15 §c§lGLOBAL RANKING", scoreLocation, true);
		...
		for (PlayerTop scoreTop : topScore) {
			if (scoreTop != null) {
				...
				if (account.isLoaded() && account != null) {
					...
					line = "§7" + scoreTop.getId() + ". §6" + name.replace("§l", "") + " §7- §b" + leagueType.getColor()
							+ leagueType.getSymbol() + " §7- §a" + df.format(scoreTop.getTop()) + " XP";

				} else {

					String name = scoreTop.getName();

					line = "§7" + scoreTop.getId() + ". §7" + name + " §7- §b" + leagueType.getColor()
							+ leagueType.getSymbol() + " §7- §a" + df.format(scoreTop.getTop()) + " XP";
				}

				scoreHologram.addLine(line);

				ss.add("§a" + scoreTop.getId() + ". " + scoreTop.getName() + " - " + leagueType.getColor()
						+ leagueType.getSymbol() + " - " + df.format(scoreTop.getTop()) + " XP");
			}
		}

		scoreHologram.addLine("   ");
		scoreHologram.addLine("§fMais status disponíveis em: §7zenix.cc/ranking");
		...
		Location scoreLocationHG = new Location(Bukkit.getServer().getWorld("world"), 9982, 101, 10020);
		scoreHG = new Hologram("§E§lTOP 15 §c§lHG WINS", scoreLocationHG, true);
		scoreHG.addLine("   ");
		...
		Location scoreLocationPvP = new Location(Bukkit.getServer().getWorld("world"), 9973, 101, 10015);
		scorePvP = new Hologram("§E§lTOP 15 §c§lPVP WINS", scoreLocationPvP, true);
		scorePvP.addLine("   ");
		...
		Location scoreLocationClans = new Location(Bukkit.getServer().getWorld("world"), 23, 71, -52);
		scoreClans = new Hologram("§e§lTOP 15 §c§lCLANS", scoreLocationClans, true);
		scoreClans.addLine("   ");

		for (PlayerTopClans scoreTop : topScoreClans) {
			if (scoreTop != null) {
				String line = "";
				String name = "§e" + scoreTop.getNameClan();

				line = "§c" + scoreTop.getId() + ". §e" + name.replace("§l", "") + " §7- §b" + scoreTop.getTop()
						+ " Xp Total";

				scoreClans.addLine(line);
			}
		}

		scoreClans.addLine("   ");
		scoreClans.addLine("§fMais status disponíveis em: §7zenix.cc/clans");

		for (Player player : Bukkit.getOnlinePlayers()) {
			scoreClans.hide(player);
			scoreClans.show(player);
		}
