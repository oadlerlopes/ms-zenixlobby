...
public class CrateManager extends Management {
	...
	public boolean initialize() {
		...
		coalHologram = new Hologram("§8Caixa de Carvão", new Location(Bukkit.getWorld("world"), 19.5, 63.5, -37.4),
				true);
		coalHologram.addLine("§aClique no baú para abrir.");

		silverHologram = new Hologram("§fCaixa de Prata", new Location(Bukkit.getWorld("world"), 19.3, 63.5, -43.5),
				true);
		silverHologram.addLine("§aClique no baú para abrir.");

		goldHologram = new Hologram("§6Caixa de Ouro", new Location(Bukkit.getWorld("world"), 16.5, 63.5, -39.5),
				true);
		goldHologram.addLine("§aClique no baú para abrir.");

		diamondHologram = new Hologram("§bCaixa de Diamante",
				new Location(Bukkit.getWorld("world"), 16.5, 63.5, -41.5), true);
		...
		executeSpiral(diamondHologram.getLocation().clone().add(0, -1, 0), ParticleType.WITCH_MAGIC, 0.3F);
		executeSpiral(diamondHologram.getLocation().clone().add(0, -1, 0), ParticleType.FLAME, 0.7F);
		executeSpiral(diamondHologram.getLocation().clone().add(0, -1, 0), ParticleType.FIREWORKS_SPARK, 0.7F);

		executeSpiral(silverHologram.getLocation().clone().add(0, -1, 0), ParticleType.WITCH_MAGIC, 0.3F);
		executeSpiral(silverHologram.getLocation().clone().add(0, -1, 0), ParticleType.FLAME, 0.7F);
		executeSpiral(silverHologram.getLocation().clone().add(0, -1, 0), ParticleType.FIREWORKS_SPARK, 0.7F);
	...
	private void givePrize(Player player, Crate crate) {
		...
		if (reward.getType() == RewardType.COINS) {
			...
			Bukkit.dispatchCommand(Bukkit.getConsoleSender(),
					"givedoublexp " + account.getUniqueId() + " " + reward.getAmmount());

			account.sendMessage("§e§lCAIXA §fVocê ganhou §a" + reward.getAmmount() + " double xp §fna caixa §b"
					+ crate.getName() );
		} else if (reward.getType() == RewardType.VIP) {
			Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "groupset " + account.getUniqueId() + " ultimate "
			...
			account.sendMessage("§e§lCAIXA §fVocê ganhou §a"
					...
					+ " de vip ultimate§f na caixa §b" + crate.getName() );
			...
			account.sendMessage("§e§lCAIXA §fVocê ganhou o kit §a" + reward.getReward() + " no "
					+ (reward.getAmmount() == 1 ? "pvp" : "hg") + "§f na caixa §b" + crate.getName() );
		}
	...
	public void executeSpiral(Location crateLocation, ParticleType effect, float radius) {
		new BukkitRunnable() {
	...
	public void executeOpsetSpiral(Location crateLocation, ParticleType effect, float radius) {
		new BukkitRunnable() {
			double xRotation, yRotation, zRotation = 5.0D;
			double angularVelocityX = 0.01570796326794897D, angularVelocityY = 0.01847995678582231D,
					angularVelocityZ = 0.0202683397005793D;
