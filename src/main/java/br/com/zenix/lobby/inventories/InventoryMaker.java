...
public class InventoryMaker extends BukkitListener {
	...
	private static final String titleOTC = "§8§nServidores de OITC";
	private static final String titlePTC = "§8§nServidores de POT";
	...
	private static final String titleGame = "§7Menu de jogos";

	public static final Inventory lobbyInventory = Bukkit.createInventory(null, 27, titleLobby);
	public static final Inventory otcInventory = Bukkit.createInventory(null, 27, titleOTC);
	public static final Inventory ptcInventory = Bukkit.createInventory(null, 27, titlePTC);
	...
	public static final Inventory gameInventory = Bukkit.createInventory(null, 27, titleGame);
	...
	public void startUpdate(Manager manager) {
		...
		new BukkitRunnable() {
			public void run() {
				...
				gameInventory.setItem(10, new ItemBuilder(Material.BOOKSHELF).setName("§aAlterar de Lobby").setDescription("§7Clique para alterar seu lobby").getStack());

				gameInventory.setItem(12, new ItemBuilder(Material.IRON_CHESTPLATE).setName("§a§lPvP")
						.setDescription(
								"\n§fTreine suas habilidades em §7combate§f e prepare-se para o §7grande confronto§f!\n\n§7"
										+ getManager().getServerManager().getServerPlayersPath("PVP")
										+ " §fjogadores conectados\n\n§a» Clique para ir ao servidor")
						.getStack());

				gameInventory.setItem(13, new ItemBuilder(Material.MUSHROOM_SOUP).setName("§a§lHungerGames")
						.setDescription(
								"\n§fSerá que você é bravo o suficiente §7para sobreviver contra 80 players?\n\n§7"
										+ getManager().getServerManager().getServerPlayersPath("HG")
										+ " §fjogadores conectados\n\n§a» Clique para ir ao servidor")
						.getStack());

				gameInventory.setItem(14,
						new ItemBuilder(Material.POTION).setName("§a§lPractice")
								.setDescription("\n§fServidor brasileiro de practice.\n\n§3§l"
										+ getManager().getServerManager().getServerPlayersPath("PRACTICE")
										+ " §fjogadores conectados\n\n§a» Clique para ir ao servidor")
								.getStack());

				gameInventory.setItem(15, new ItemBuilder(Material.IRON_FENCE).setName("§a§lGladiator")
						.setDescription(
	...
	public ItemBuilder getItemMinigame(FullyServerStatus server) {
		return new ItemBuilder(Material.PAPER)
				.setName("§a§L" + server.getName().replace("OITC-", "OITC (").replace("SW-", "SW (") + ")")
	...
	private void onInventoryClick(InventoryClickEvent event) {
		if (event.getInventory().getTitle() != null && event.getCurrentItem() != null) {
			...
			if (event.getInventory().getTitle().equalsIgnoreCase(titlePvP)) {
				...
				} else if (type.equals(Material.POTION)) {
					if (right) {
						getManager().getServerManager().randomServer(player, "practice-1");
						return;
					}
					getManager().getServerManager().connectPlayer(player, "practice-1");
			...
			if (!type.equals(Material.INK_SACK) && !type.equals(Material.IRON_CHESTPLATE)
					&& !type.equals(Material.DIAMOND_SWORD) && !type.equals(Material.PAPER)
